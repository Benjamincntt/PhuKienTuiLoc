# Shared helpers: read SQL connection from appsettings.Development.json or env vars.
# Env override: PKTL_SQL_SERVER, PKTL_SQL_DATABASE, PKTL_SQL_USER, PKTL_SQL_PASSWORD
...
function Get-DbSettings {
    ...
    $settingsPath = Join-Path $root "backend\appsettings.Development.json"

    $server = $env:PKTL_SQL_SERVER
    $database = $env:PKTL_SQL_DATABASE
    $user = $env:PKTL_SQL_USER
    $password = $env:PKTL_SQL_PASSWORD
    ...
    if (-not $database) { $database = "PhuKienTuiLocDb" }
...
function Get-DbBackupPath {
    ...
    Join-Path $dir "PhuKienTuiLocDb.bak"
}
